 ...
 *
 * Steps: pj_description → pj_profession → pj_location → pj_address → pj_my_cut → pj_sub_pay → pj_confirm
 */

const SUPA_URL = process.env.SUPABASE_URL || '';

/** Start the publish job flow */
export async function startPublishJob(phone: string, state: BotState): Promise<void> {
  ...
  await sendText(phone,
    '📝 *Post a Job*\n\nDescribe the job you need done.\n\nExample: "Locksmith needed for residential lock change, 3 locks"'
  );
}
...
async function handleDescription(phone: string, text: string, state: BotState, job: Record<string, string>) {
  ...
  await setState(phone, state);
  await sendText(phone,
    '🔧 What trade is this?\n\nExamples: locksmith, plumber, electrician, HVAC, painter, roofer'
  );
}

async function handleProfession(phone: string, text: string, state: BotState, job: Record<string, string>) {
  if (!text.trim()) {
    await sendText(phone, 'Please enter a trade (e.g. locksmith, plumber).');
    return;
  }
  job.profession = text.trim().toLowerCase().replace(/\s+/g, '_');
  state.extra = { ...(state.extra || {}), job };
