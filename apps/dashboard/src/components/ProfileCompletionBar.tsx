...
export default function ProfileCompletionBar({ completeness, profile }: ProfileCompletionBarProps) {
  ...
  const actionTasks: ActionTask[] = [
    {
      ...
      hidden: pwa.isInstalled || (!pwa.canInstall && !pwa.isIOS),
    },
    {
      key: 'enable_push',
      ...
      hidden: push.isUnsupported || push.hasPermission,
    },
  ]
  ...
  // Compute actual completeness from checklist (including action tasks)
  const checklistEarned = CHECKLIST.reduce(
    (sum, item) => sum + (item.check(profile) ? item.points : 0),
    0,
  )
  const earned = checklistEarned + actionEarned
  const percent = Math.min(
    Math.round((earned / (TOTAL_POINTS + actionTotal)) * 100),
    100,
  )
