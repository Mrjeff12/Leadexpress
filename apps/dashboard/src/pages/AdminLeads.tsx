...
export default function AdminLeads() {
  ...
  // This is used to calculate the correct KPI numbers.
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (contractorsRes.data && subsRes.data) {
        const activeSubUserIds = new Set(subsRes.data.map(s => s.user_id))
        const mapped = contractorsRes.data.map(c => ({
          professions: c.professions,
          zip_codes: c.zip_codes,
          is_active: c.is_active,
          subscription_status: activeSubUserIds.has(c.user_id) ? 'active' : 'inactive'
        }))
  ...
  // Filter
  // (Removed old filter logic since it's now handled by baseFilteredLeads and filtered useMemos)

  const getPotentialCount = (lead: Lead) => {
    return contractors.filter(c => {
      const hasProf = c.professions.includes(lead.profession)
      const hasZip = lead.zip_code ? c.zip_codes.includes(lead.zip_code) : true
      const isActiveSub = c.subscription_status === 'active'
      return hasProf && hasZip && isActiveSub
    }).length
  }
      ...
      {!loading && filtered.length > 0 && (
        <div className="space-y-4">
          {filtered.map((lead) => {
            ...
            const potentialCount = getPotentialCount(lead)
                ...
                <div className="flex items-stretch min-h-[100px]">
                  ...
                  <div className="w-64 flex flex-col justify-center p-6 border-l border-black/[0.03] shrink-0 gap-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-black/[0.03] border border-black/5">
                        <span className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">Potential</span>
                        <span className="text-xs font-bold text-black flex items-center gap-1">
                          <Users className="w-3 h-3" /> {potentialCount}
                        </span>
                      </div>
                      <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-black/[0.03] border border-black/5">
                        <span className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">Delivered</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-3">
                          <button className="flex-1 flex items-center justify-center gap-2 bg-black text-white h-12 rounded-2xl font-bold text-sm hover:bg-stone-800 transition-all">
                            <Send className="w-4 h-4" />
                            {he ? 'שלח לקבלנים' : 'Distribute Lead'}
                          </button>
                          <button className="w-12 h-12 flex items-center justify-center rounded-2xl bg-black/5 hover:bg-black/10 transition-all">
